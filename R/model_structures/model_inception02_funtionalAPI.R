...
tower2 <- input_conv %>%
  ...
  layer_activation("relu")%>%
  layer_batch_normalization() %>%
  layer_conv_2d(filter=64, padding = "same", kernel_size=c(3,3), kernel_initializer = "lecun_uniform", strides=c(1,1),use_bias=T) %>%
  layer_activation("relu") %>%
  layer_batch_normalization() %>%
...
tower3 <- input_conv %>%
  ...
  layer_activation("relu")%>%
  layer_batch_normalization() %>%
  layer_conv_2d(filter=64, padding = "same", kernel_size=c(5,5), kernel_initializer = "lecun_uniform", strides=c(1,1),use_bias=T) %>%
  layer_activation("relu") %>%
  layer_batch_normalization() %>%
  layer_max_pooling_2d(pool_size=c(2,2))%>%
  layer_flatten()
