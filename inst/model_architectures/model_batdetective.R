...
model %>%
  keras::layer_conv_2d(filter=32, kernel_size=c(3,3), padding="valid", input_shape=input_shape, strides=c(1,1), kernel_initializer = "lecun_uniform", use_bias=T) %>%
  ...
  keras::layer_dense(256, use_bias=T) %>%
  keras::layer_activation("relu") %>%
  keras::layer_batch_normalization() %>%
  keras::layer_dropout(0.25) %>%
