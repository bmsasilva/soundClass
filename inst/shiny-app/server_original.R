...
shinyServer(function(input, output, session) {
  ...
  # Escolher pasta com as gravacoes -----------------------------------------

  # Definir o ponto de partida para o explorador
  # Para partir do directorio actual usar: volumes <- c(wd='.')
  volumes <- getVolumes()
 # volumes <- c(wd='.')

  # Parte do server correspondente ao shinyDirButton
  shinyDirChoose(input, 'folder', session=session, roots=volumes)
  ...
  # Importar gravacao e converter em objecto da classe BatRecorder ----------
  som <- reactive({
...
- Butterworth filters are applied to zoomed area
...
- Adjust spectrogram settings with:
    ...
    # Criar objecto da classe BatRecorder
    som <- new('BatRecording', input$files)# mudar isto de forma a ir buscar o path da directoria escolhida

    # Aplicar as definicoes escolhidas pelo utilizador
    som <- setOptions(som,  windowLength = as.numeric(input$windowLength),
    ...
    som <- buttFilter(som, ranges$y[1], ranges$y[2])
  ...
  output$spec <- renderPlot({
    powSpec(som(), xlim = ranges$x, ylim = ranges$y)
  })
