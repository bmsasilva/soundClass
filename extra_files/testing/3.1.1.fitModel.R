...
path<- '/media/DADOS/BRUNO/A2020/1.Geraldina/database/'

load(paste0(path,'bats_win1ms_step0.25ms_res1khz_thresh90.RData'))
load(paste0(path,'noise_win1ms_step0.25ms_res1khz_thresh90.RData'))
...
model %>%  
  layer_conv_2d(filter=32, kernel_size=c(3,3), padding="valid", input_shape=input_shape, strides=c(1,1), kernel_initializer = "lecun_uniform" ) %>%  
  layer_activation("relu") %>%  
  layer_max_pooling_2d(pool_size=c(2,2)) %>% 
  #layer_dropout(rate = 0.25) %>% 

  layer_conv_2d( filter=64, kernel_size=c(3,3), kernel_initializer = "lecun_uniform", strides=c(1,1)) %>%  
  layer_activation("relu") %>%  
  layer_max_pooling_2d(pool_size=c(2,2)) %>% 
  #layer_dropout(rate = 0.25) %>% 
  
  layer_conv_2d( filter=128, kernel_size=c(3,3), kernel_initializer = "lecun_uniform", strides=c(1,1)) %>%  
  layer_activation("relu") %>%  
  layer_max_pooling_2d(pool_size=c(2,2)) %>% 
  #layer_dropout(rate = 0.25) %>% 
...
model %>%
compile( 
  optimizer = optimizer_sgd(lr=0.001, momentum=0.9, nesterov=T),
...
save_model_hdf5(model, './models/model_triagem_24OUT2017.hdf5', overwrite = TRUE,
                include_optimizer = TRUE)

model_triagem<-load_model_hdf5('./models/model_triagem_24OUT2017.hdf5')
