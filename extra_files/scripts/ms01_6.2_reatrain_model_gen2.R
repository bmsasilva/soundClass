...
load("../recLabel_database/ms01/datasets/new_train_matrix_retrain_gen2.RDATA") 
...
aux <- gsub(pattern = "Nleis", replacement = 14, x = aux)
...
model <- load_model_hdf5("~/Projectos/R_packages/recLabel_database/ms01/gen02_model/modelo_inception02_lr0.01_gen2_06-0.9994.hdf5")
summary(model)
...
history<-model %>% fit(re_train_x, re_train_y, 
                       ...
                       callbacks = list(callback_early_stopping(patience = 1, monitor = 'acc'),
                                        callback_model_checkpoint("../recLabel_database/ms01/gen03_model/modelo_inception02_lr0.01_gen3_{epoch:02d}-{acc:.4f}.hdf5", monitor = "acc")),
                       shuffle = TRUE,
                       verbose = 1)

save(history, file="../recLabel_database/ms01/gen03_model/history_model_lr0.01_gen03.RDATA")
