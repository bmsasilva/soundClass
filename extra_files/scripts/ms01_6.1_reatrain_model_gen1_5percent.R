...
load("../recLabel_database/ms01/datasets/new_train_matrix_retrain_base_model_5percent.RDATA") 
...
history<-model %>% fit(re_train_x, re_train_y, 
                       batch_size = 64, 
                       epochs = 15,
                       callbacks = list(callback_early_stopping(patience = 2, monitor = 'acc'),
                                        callback_model_checkpoint("../recLabel_database/ms01/gen02_model/modelo_inception02_lr0.01_gen2_5percent_{epoch:02d}-{acc:.4f}.hdf5", monitor = "acc")),
                       shuffle = TRUE,
                       verbose = 1)

save(history, file="../recLabel_database/ms01/gen02_model/history_model_lr0.01_gen02_5percent.RDATA")
