...
load("../recLabel_database/ms01/datasets/new_train_matrix_retrain_handRelesase_5percent.RDATA") 
...
history<-model %>% fit(re_train_x, re_train_y, 
                       ...
                       callbacks = list(callback_early_stopping(patience = 1, monitor = 'acc'),
                                        callback_model_checkpoint("../recLabel_database/ms01/gen02_model/modelo_inception02_lr0.01_gen2_5percent_handRelease_{epoch:02d}-{acc:.4f}.hdf5", monitor = "acc")),
                       shuffle = TRUE,
                       verbose = 1)

save(history, file="../recLabel_database/ms01/gen02_model/history_model_lr0.01_gen02_5percent_handRelease.RDATA")
